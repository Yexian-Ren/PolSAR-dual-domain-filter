...
% x and y are the top left corner locations of the selected non-texture homogeneous regions
% and then a 15*15 window is opened to calculate the cut-off threshold.
x=[710,944,959,785]';
y=[398,191,26,7]';%Input coordinates of non-textured homogeneous regions
alpha = 0.6;%fixed

% Parameter for SAR-POTDF
% theoretic ENL = 3*the number of the looks of the original PolSAR data
% empirical value
look = 1; %real number of the looks of the original PolSAR data
if look == 1
    L = 2.75;%fixed
elseif look == 4
    L = 7.5;
...
%%  simple evaluation
[ratio,enl2,entropy,alpha,A,y] = basis_filter_evaluate(T3mat,T3mat_dd,4);
